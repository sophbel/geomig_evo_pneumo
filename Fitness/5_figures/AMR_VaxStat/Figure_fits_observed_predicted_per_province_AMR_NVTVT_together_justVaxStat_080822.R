...
# fit = readRDS(file = '4_run_model/AMR_VaxStat/AMR_noGrowth/Output_vaxstatAMR_NVT_amrNG.R_fit_all.rds')

## Chains
Chains=rstan::extract(fit$fit)

nb_countries = fit$data$nb_countries
nb_genotypes = fit$data$nb_genotypes_vstat
nb_years = fit$data$nb_years

ref_clade = 1
### vstat
# numbers_obs_all_clades_vstat<-numbers_obs_all_clades <- array(NA, dim = c(nb_genotypes, nb_years, nb_countries)) 
# for(i in 1:nb_countries){
#   numbers_obs_all_clades_vstat[1:(nb_genotypes-1),,i] = fit$data$data_genotype_ref_vstat[,,i]
#   numbers_obs_all_clades_vstat[nb_genotypes,,i] = fit$data$data_genotype_non_ref_vstat[,i]
# }
### VT
# numbers_obs_all_clades = array(NA, dim = c(nb_genotypes, nb_years, nb_countries)) 
# for(i in 1:nb_countries){
#   numbers_obs_all_clades_vt[1:(nb_genotypes-1),,i] = fit$data$data_genotype_non_ref_vt[,,i]
#   numbers_obs_all_clades_vt[nb_genotypes,,i] = fit$data$data_genotype_ref_vt[,i]
# }
threshold = 5
...
############################################################################################
## Plot fits for Vaccine Status NVT and VT
############################################################################################

# ## Chains
pdf(width = 3*2, height = 3, file = "./5_figures/AMR_VaxStat/Figure_fits_VaxStat_2010_overall.pdf", onefile = T)
par(mfcol = c(1,2), oma = c(1,1,1,0), mai = c(0.2,0.4,0.2,0.4))

colors = c('maroon', 'darkblue')
mat.list<-list()
for(c in 1:1){
  ...
  count = 1
  d = rowSums(fit$data$data_genotype_non_ref_vstat[1,,])
  ...
  for(i in 1:(nb_genotypes)){
    ...
    # t = rowSums(colSums(fit$data$data_genotype_total))
    t = rowSums(fit$data$data_genotype_ref_vstat)+rowSums(fit$data$data_genotype_non_ref_vstat[1,,])
    d_m = rep(0, length(d))
    d_ci = matrix(0, ncol = length(d), nrow = 2)
    if(i < nb_genotypes){
      d = rowSums(fit$data$data_genotype_ref_vstat)
      for(j in 1:nb_years){
        if(t[j]>0) {
    ...
    if(i == nb_genotypes){
      d = rowSums(fit$data$data_genotype_non_ref_vstat[i-1,,])
      for(j in 1:nb_years){
        if(t[j]>0) {
          tmp = binom.confint(x = d[j], n = t[j], method = c("bayes"), type="central")
          d_m[j] = tmp$mean
    ...
    f = matrix(0, nrow = nb_chains, ncol = length(1:nb_years))
      for(ccc in 1:nb_countries){
        f = f + t(apply(Chains$pred_absolute_freq_vstat[,ccc,i,1:nb_years],MARGIN = 1, function(x)x*sum(colSums(fit$data$data_genotype_total)[,ccc])/sum(t)))
...
# pdf(width = 2.83, height = 3.67, file = "/Users/sb62/Documents/Migration/SA_Migration_110422/Fitness/RunModel/AMR_VaxStat/Fits_Estimates/Figure_fits_AMR_VT_2010_overall.pdf", onefile = T)
# par(mfcol = c(1,2), oma = c(1,1,1,0), mai = c(0.2,0.2,0.2,0.1))
#
...
# setwd("/Users/sb62/Documents/Migration/SA_Migration_110422/Fitness/RunModel/AMR_VaxStat/indivfits/")
