...
############# R(t) model
MCMC_fitness_NVT_PCV7_PCV13_2switches_1pervax.Rmodel.MCMC <- stan_model(file = '../3_model/Model_fitness_PCV_2p_vaccineintro_switch_per_vax_22012024.stan')

############# data for MCMC ######################################################
data.MCMC = readRDS('../2_processed_data/Data_model_NVT_PCV7_PCV13_11082022_ref_NVT_disease.rds')
...
for(delay in delays){
  ...
  if(delay >= 0) name_file = paste0('Output_per_provice_NVT_PCV7_PCV13_swicthes2009and2011_1pervax_disease_plus', abs(delay))
  if(delay < 0) name_file = paste0('Output_per_provice_NVT_PCV7_PCV13_swicthes2009and2011_1pervax_disease_minus', abs(delay))
  print(name_file)
  ...
  foreach(i = 1:3)  %dopar% {
    ...
    fit_delay <- sampling(model.MCMC, data = data.MCMC,
                          ...
                          init = list(list(f0 = f0_init(data.MCMC$nb_countries, data.MCMC$nb_genotypes),
                                           fitness_genotypes_pre_vacc_PCV7 = matrix(rnorm(1,0,0.1), ncol = 1, nrow = 1),
                                           fitness_genotypes_post_vacc_PCV7 = matrix(rnorm(1,0,0.1), ncol = 1, nrow = 1),
                                           fitness_genotypes_pre_vacc_PCV13 = matrix(rnorm(1,0,0.1), ncol = 1, nrow = 1),
                                           fitness_genotypes_post_vacc_PCV13 = matrix(rnorm(1,0,0.1), ncol = 1, nrow = 1))))  # iter =10000 et plus de chaines
