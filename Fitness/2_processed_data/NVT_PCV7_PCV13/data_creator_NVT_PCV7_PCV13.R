...
nb_countries = length(levels(as.factor(GPS_SA.sub$Region)))
countries = levels(as.factor(GPS_SA.sub$Region))
...
for(i in 1:nb_countries){
  for(j in 1:nb_years){
    clade_number_array[,j,i] = table(factor(GPS_SA.sub$Vaccine_type[which(GPS_SA.sub$Col_Year == first_year+(j-1) & GPS_SA.sub$Region == countries[i])], levels = c('NVT', 'PCV7', 'PCV13')))
  }
}
