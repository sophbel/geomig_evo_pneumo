...
data {
  ...
  int number_introductions;
	int yearIntroduction[number_introductions]; // dates of introdution for each country
	int vax_implementation[dim_data-1];
  ...
  for (j in 1:(dim_data-1)){
    if(fitness_genotypes_vector_pre_vacc[j] < 20){
      if(vax_implementation[j] > 0){
        pred_paired_freq[j,] = logistic_model(f0[j]/(f0[j]+f0[dim_data]), fitness_genotypes_vector_pre_vacc[j], fitness_genotypes_vector_post_vacc[j], nb_years, yearF0, yearIntroduction[vax_implementation[j]]);
      }
      if(vax_implementation[j] == 0){
        pred_paired_freq[j,] = logistic_model(f0[j]/(f0[j]+f0[dim_data]), fitness_genotypes_vector_pre_vacc[j], fitness_genotypes_vector_post_vacc[j], nb_years, yearF0, yearIntroduction[1]);
      }
    }
    if(fitness_genotypes_vector_pre_vacc[j] > 20){ // Reference serotype
      pred_paired_freq[j,] = logistic_model(f0[j]/(f0[j]+f0[dim_data]), 0, 0, nb_years, yearF0, yearIntroduction[1]);
    }
  }
  for(l in 1:nb_years){
	    for(j in 1:(dim_data-1)){
...
generated quantities {
  // vector[(dim_data-1)*yearIntroduction-number_zeros_country_year_genotype] log_lik;
