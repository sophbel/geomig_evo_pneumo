functions {
	real[] logistic_model(real y0, real fitness_clades_pre_vacc, real fitness_clades_post_vacc, int nb_years, int vaccine_introduction, real ACV_introduction){
		real res[nb_years];
		real tmp;
    tmp = y0;
...
data {
  ...
  // ACV implementation
  int number_introductions;
	real yearIntroduction[number_introductions]; // dates of introdution for each country
	
	int index_paramater[nb_genotypes-1];
	int vax_implementation[nb_genotypes-1];
	int nb_vaccine_parameters;
...
parameters {
	real<lower=-5, upper=5> fitness_genotypes_pre_vacc[nb_vaccine_parameters, number_R_pre_vacc];
	real<lower=-5, upper=5> fitness_genotypes_post_vacc[nb_vaccine_parameters, number_R_post_vacc];
	
	simplex[nb_genotypes] f0[nb_countries];
}

transformed parameters{
  real<lower=-5, upper=5> fitness_genotypes_vector_pre_vacc[nb_vaccine_parameters, number_R_pre_vacc];
	real<lower=-5, upper=5> fitness_genotypes_vector_post_vacc[nb_vaccine_parameters, number_R_post_vacc];
  ...
  // compute predicted numbers
	for (k in 1:nb_countries){
	  ...
	  for (j in 1:(nb_genotypes-1)){
	    if(index_paramater[j] > 0){
	      if(vax_implementation[j] > 0){
	        pred_paired_freq[k,j,] = logistic_model(f0[k,j]/(f0[k,j]+f0[k,nb_genotypes]), fitness_genotypes_vector_pre_vacc[index_paramater[j],1], fitness_genotypes_vector_post_vacc[index_paramater[j],1], nb_years, yearF0, yearIntroduction[vax_implementation[j]]);
	      }
	      if(vax_implementation[j] == 0){
	        pred_paired_freq[k,j,] = logistic_model(f0[k,j]/(f0[k,j]+f0[k,nb_genotypes]), fitness_genotypes_vector_pre_vacc[index_paramater[j],1], fitness_genotypes_vector_post_vacc[index_paramater[j],1], nb_years, yearF0, yearIntroduction[1]);
	      }
	    }
	    if(index_paramater[j] == 0){
	      pred_paired_freq[k,j,] = logistic_model(f0[k,j]/(f0[k,j]+f0[k,nb_genotypes]), 0, 0, nb_years, yearF0, yearIntroduction[1]);
	    }
	  }
    for(l in 1:nb_years){
  	    for(j in 1:(nb_genotypes-1)){
...
model {
  for (i in 1:nb_vaccine_parameters){
