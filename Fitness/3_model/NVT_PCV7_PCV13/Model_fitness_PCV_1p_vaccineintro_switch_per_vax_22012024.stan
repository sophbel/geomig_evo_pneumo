...
parameters {
	real<lower=-5, upper=5> fitness_genotypes_pre_vacc[1, number_R_pre_vacc];
	real<lower=-5, upper=5> fitness_genotypes_post_vacc[1, number_R_post_vacc];

	simplex[nb_genotypes] f0[nb_countries];
}

transformed parameters{
  real<lower=-5, upper=5> fitness_genotypes_vector_pre_vacc[1, number_R_pre_vacc];
	real<lower=-5, upper=5> fitness_genotypes_vector_post_vacc[1, number_R_post_vacc];
  ...
  // compute predicted numbers
	for (k in 1:nb_countries){
	  ...
	  for (j in 1:(nb_genotypes-1)){
	    if(j==1){
	      pred_paired_freq[k,j,] = logistic_model(f0[k,j]/(f0[k,j]+f0[k,nb_genotypes]), fitness_genotypes_vector_pre_vacc[1,1], fitness_genotypes_vector_post_vacc[1,1], nb_years, yearF0_PCV7[k], yearIntroduction_PCV7[k]);
	    }
	    if(j==2){
	      pred_paired_freq[k,j,] = logistic_model(f0[k,j]/(f0[k,j]+f0[k,nb_genotypes]), fitness_genotypes_vector_pre_vacc[1,1], fitness_genotypes_vector_post_vacc[1,1], nb_years, yearF0_PCV13[k], yearIntroduction_PCV13[k]);
	    }
	  }
    for(l in 1:nb_years){
  	    for(j in 1:(nb_genotypes-1)){
...
model {
  fitness_genotypes_post_vacc[1,1] ~ cauchy(0, 0.15);
  fitness_genotypes_pre_vacc[1,1] ~ cauchy(fitness_genotypes_post_vacc[1,1], 0.15);
