...
for (boot in 1:3){
  load(paste0("./MCMC_model/Simulations/output/true_downSamp/sim.ans.",scale,".",iters,".",boot,"_adj.RData"))
  load(paste0("./MCMC_model/Simulations/output/true_downSamp/dat.in2",scale,".",iters,".",boot,"_adj.RData"))
  load(paste0("./MCMC_model/Simulations/output/true_downSamp/dat.in.all",scale,".",iters,".",boot,"_adj.RData"))
...
for(post in 1:nsim){
  ...
  df<-t(quantiles.probInfec)
  df<-data.frame(df)
  df$truth<-nInfecLoc.true
  colnames(df)<-c("lowerCI","med","upperCI","truth")
  df$popsize<-extTranMatDat.tmp$popbyCell/1000000
  
  ggplot()+
    geom_point(data=df,aes(x=popsize,y=med),size=3,alpha=0.8)+
    geom_errorbar(data=df,aes(x=popsize,ymin=lowerCI,ymax=upperCI),width=0)+
    geom_point(data=df,aes(x=popsize,y=truth),size=3,color="purple",alpha=0.8)+
    theme_classic()+
  ...
  # pdf(file=paste0("./MCMC_model/Simulations/Plots/infecProbLoc",boot,".pdf"),height=4,width=4)
...
simFit
ggsave(filename = paste0("./MCMC_model/Simulations/Plots/simFit",boot,".pdf"))
}
