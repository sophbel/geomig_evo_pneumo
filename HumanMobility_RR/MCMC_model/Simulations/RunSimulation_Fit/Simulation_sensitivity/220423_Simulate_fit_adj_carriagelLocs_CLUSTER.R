...
ntimes<-90000 ## No. of pairs per simulation pre observation
...
###SUB-Sample those from places with carriage only
probSampleCar<-vector(mode="numeric",length=9)
probSampleCar[c(3,6)]<-0.5
samp1<-rbinom(nrow(dat.in.all),1,prob=probSampleCar[dat.in.all[,"loc1"]])
samp2<-rbinom(nrow(dat.in.all),1,prob=probSampleCar[dat.in.all[,"loc2"]])
...
save(dat.in2,file=paste0("./MCMC_model/Simulations/output/carriageLocs/dat.in2",scale,".",iters,".",boot,"_adj2.RData"))
save(dat.in.all,file=paste0("./MCMC_model/Simulations/output/carriageLocs/dat.in.all",scale,".",iters,".",boot,"_adj2.RData"))
save(sim.ans,file=paste0("./MCMC_model/Simulations/output/carriageLocs/sim.ans.",scale,".",iters,".",boot,"_adj2.RData"))###SAVE OUTPUT
}
###############################################################################
